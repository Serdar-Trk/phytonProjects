...
# Sadece numeric ya da sadece kategorik degiskenler uzerinde islem yapmak istersek
import numpy as np

df.select_dtypes(include=np.object_).dtypes
df.select_dtypes(include=np.number).dtypes
df.select_dtypes(include=np.bool_).dtypes
...
# bu durumda 'sex' degiskeni index olmus olur onu tekrar degisken yapmak istersek, 'reset_index()
df.reset_index()
# eger index e attigimiz degiskenin ismini kaldirmak istersek asagidaki islemi uygulariz
...
# crosstab kullanimi
# iki degiskenin korelasyonu
pd.crosstab(df['sex'], df['survived'])

# replace islemi
df.head()
df['who'].str.replace('n', '')
df['who'].str.replace('[/w/n]', 'c')

# null verileri silmek 'dropna'
df.isnull().sum()
df.dropna(subset=['deck', 'embarked'], how='any', inplace=True)
df['age'].fillna(df['age'].mean(), inplace=True)
# deck ve embarked degiskenlerinde 'null' deger iceren butun row lari sildik.
# daha sonra 'null' deger iceren age row larini age degerlerinin ortalamasi ile doldurduk
...
# degiskenlerin gpu kulllanimlari
# bunu iyilestirmek icin ne yapabiliriz?
df.memory_usage(deep=True)

df.dtypes

#  unique deger sayisi az olan(sik tekrar eden) object yapilari category yapisina ceviririz.

df['sex'].nunique()
df['sex'].value_counts()
df['sex'] = df['sex'].astype('category')

# bu islem ile 'sex' degiskeninin gpu kullanimini 55000 den 1000 e dusurmus olduk.
# unique deger sayisi cok yuksek olan degerleri object yapisina ceviririz


# category yapisinda siralamak istedigimiz veriyi istedigimiz dogrultuda yapilandirabiliriz.
q = pd.DataFrame({'id': [100, 101, 102, 103], 'quality': ['good', 'very_good', 'good', 'excellent']})
q.dtypes

q['quality'].sort_values()
#  goruldugu uzere alfabetik olarak kucukten buyuge siraladi. Peki biz kalitesine gore siralamasini istersek.

from pandas.api.types import CategoricalDtype

cat_dtype = CategoricalDtype(categories=['good', 'very_good', 'excellent'], ordered=True)
q['quality'] = q['quality'].astype(cat_dtype)

q['quality'].sort_values()
# boylece kategorik bir alanda asagida goruldugu gibi bir kosul belirtebiliriz.
q.loc[q['quality'] > 'good', :]

# ayri bir ornek
from pandas.api.types import CategoricalDtype

cat_dtype = CategoricalDtype(categories=[1, 2, 3], ordered=True)
df['pclass'] = df['pclass'].astype(cat_dtype)
df['pclass'].sort_values()
